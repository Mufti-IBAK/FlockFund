...
export default function NewFundRequestPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data } = await supabase.from('flocks').select('id, flock_name, name').eq('status', 'active');
      setFlocks(data || []);
      if (data && data.length > 0) setForm(f => ({ ...f, flock_id: data[0].id }));
    }
    load();
  }, []);
  ...
  return (
    <div className="max-w-2xl mx-auto py-8">
      <div className="mb-8">
        <h1 className="text-2xl font-heading font-extrabold text-primary tracking-tight">New Fund Request</h1>
        <p className="text-slate-400 text-sm mt-1">Specify what you need and for which flock</p>
      </div>

      <div ref={formRef} className="bg-white rounded-2xl border border-slate-200/80 p-6 md:p-8 shadow-sm">
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-1.5">
            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Target Flock</label>
            <select 
              value={form.flock_id}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Amount Required (₦)</label>
              <input 
                type="number" 
                placeholder="0.00"
                value={form.amount}
          ...
          <div className="space-y-1.5">
            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Justification / Description</label>
            <textarea 
              rows={4}
