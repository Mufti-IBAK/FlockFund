  ...
  useEffect(() => {
    if (contentRef.current) {
      const ctx = gsap.context(() => {
        gsap.fromTo(
          contentRef.current!.querySelectorAll(".data-card"),
          { y: 30, opacity: 0 },
          {
            y: 0,
            opacity: 1,
    ...
    <div ref={contentRef}>
      <div className="mb-8">
        ...
        <p className="text-slate-400 text-sm mt-1">
          Feed conversion ratio analytics and data collection metrics
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 grid-cols-3 gap-5 mb-8">
        {[
          ...
          {
            label: "FCR Calculations",
            value: fcrData.length,
            icon: "calculate",
          ...
          <div
            key={kpi.label}
            className={`data-card bg-gradient-to-br ${kpi.color} rounded-2xl p-5 border border-white/40`}
          >
          ...
          <div className="flex items-end gap-1 h-32">
            {fcrData.map((d, i) => {
              ...
              return (
                ...
                >
                  <div
                    className="w-full bg-accent/30 rounded-t relative group"
                    style={{
                      height: `${(d.fcr / maxFcr) * 100}%`,
                      minHeight: "4px",
                    }}
                  >
        ...
        {loading ? (
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-sm text-slate-400">
              No FCR calculations yet. Data will appear once keeper reports
              accumulate feed and weight records.
            </p>
          </div>
        ) : (
