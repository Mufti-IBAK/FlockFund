    ...
    <div ref={contentRef}>
      <div className="mb-8">
        ...
        <p className="text-slate-400 text-sm mt-1">
          Farm reports from keepers across all flocks
        </p>
      </div>
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {[
          ...
          >
            ...
            {tab.label}
            <span
              className={`ml-1 px-1.5 py-0.5 rounded-full text-[9px] ${filterStatus === tab.key ? "bg-white/20" : "bg-slate-100"}`}
            >
                ...
                >
                  <div className="flex justify-between items-start mb-1">
                    ...
                    <div className="text-right">
                      <span
                        className={`text-[9px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-md ${STATUS_COLORS[r.status]}`}
                      >
                        {r.status}
                      </span>
                    </div>
                  ...
                  <div className="grid grid-cols-3 gap-2 mt-1">
                    <div className="bg-slate-50 rounded-xl p-3 flex flex-col items-center justify-center text-center">
                      ...
                      <p
                        className={`font-mono font-bold text-sm ${r.mortality_count > 0 ? "text-rose-600" : "text-emerald-600"}`}
                      >
                        {r.mortality_count}
                      </p>
                    </div>
                    ...
                    <div className="bg-slate-50 rounded-xl p-3 flex flex-col items-center justify-center text-center">
                      ...
                      <p className="font-mono font-bold text-primary text-sm">
                        {r.feed_consumed_kg ? `${r.feed_consumed_kg}kg` : "—"}
                      </p>
                    </div>
                  </div>

                  <div className="bg-slate-50 rounded-xl p-3 flex justify-around mt-1">
                    <span
                      className={`text-xs font-medium px-2 py-1 rounded-md flex items-center gap-1 ${r.feed_available ? "bg-emerald-100 text-emerald-700" : "bg-rose-100 text-rose-700"}`}
                    >
                      <span className="material-symbols-outlined text-[14px]">
                        {r.feed_available ? "check_circle" : "cancel"}
                      </span>
                      Feed
                    </span>
                    <span
                      className={`text-xs font-medium px-2 py-1 rounded-md flex items-center gap-1 ${r.water_available ? "bg-emerald-100 text-emerald-700" : "bg-rose-100 text-rose-700"}`}
                    >
                      <span className="material-symbols-outlined text-[14px]">
                        {r.water_available ? "check_circle" : "cancel"}
                      </span>
                      Water
                    </span>
                  </div>
