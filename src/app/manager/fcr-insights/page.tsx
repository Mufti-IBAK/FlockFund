...
function FCRLineChart({ data }: { data: FCRWeek[] }) {
  ...
  const areaD =
    pathD +
    ` L ${points[points.length - 1].x} ${H - PAD_B} L ${points[0].x} ${H - PAD_B} Z`;
      ...
      >
        ...
        {points.map((p, i) => (
          <g key={i}>
            ...
            >
              <title>
                Week {p.week} · FCR: {p.fcr.toFixed(2)} · Feed:{" "}
                {p.feed.toFixed(1)}kg · Weight: {p.wt.toFixed(2)}kg
              </title>
            </circle>
          </g>
        ))}
        {data.map((d, i) => {
          const x =
            PAD_L + (i / Math.max(1, data.length - 1)) * (W - PAD_L - PAD);
          return (
            <text
              key={i}
              x={x}
        ...
        <text
          ...
          fontSize="10"
          fontWeight="bold"
          transform={`rotate(-90, 14, ${(H - PAD_B + PAD) / 2})`}
        >
          FCR Ratio
        </text>
      </svg>
      ...
      >
        ...
        {data
          ...
          .map((d) => {
            ...
            const x =
              PAD_L + (idx / Math.max(1, data.length - 1)) * (W - PAD_L - PAD);
            const short = new Date(d.date + "T00:00:00").toLocaleDateString(
              "en-US",
...
export default function ManagerFCRInsights() {
      ...
      try {
        ...
        try {
          ...
          if (selectedFlock !== "all") {
            fcrQuery = fcrQuery.eq("flock_id", selectedFlock);
          }

          const { data: fcr } = await fcrQuery;

          if (fcr && fcr.length > 0) {
            if (selectedFlock === "all") {
              const byWeek: Record<
                number,
                { w: number; f: number; fcr: number; count: number }
              > = {};
              fcr.forEach((r) => {
                const w = r.week_number;
                if (!byWeek[w]) byWeek[w] = { w: 0, f: 0, fcr: 0, count: 0 };
                byWeek[w].w += r.avg_weight_kg || 0;
                byWeek[w].f += r.total_feed_kg || 0;
                byWeek[w].fcr += r.fcr || 0;
                byWeek[w].count += 1;
              });
              fcrRows = Object.keys(byWeek)
                .map(Number)
                .sort((a, b) => a - b)
                .map((week) => ({
                  week,
                  avgWeight: byWeek[week].w / byWeek[week].count,
                  totalFeed: byWeek[week].f / byWeek[week].count,
                  fcr: byWeek[week].fcr / byWeek[week].count,
                }));
            } else {
              fcrRows = fcr.map((r) => ({
                week: r.week_number,
        ...
        if (fcrRows.length === 0) {
          let reqQuery = supabase
            ...
            .limit(500);
          if (selectedFlock !== "all")
            reqQuery = reqQuery.eq("flock_id", selectedFlock);
          const { data: reports } = await reqQuery;
          ...
          let wQuery = supabase
            ...
            .limit(300);
          if (selectedFlock !== "all")
            wQuery = wQuery.eq("flock_id", selectedFlock);
          ...
          if (reports && reports.length > 0) {
            ...
            for (const r of reports) {
              const days = Math.floor(
                (new Date(r.report_date).getTime() - firstDate.getTime()) /
                  86400000,
              );
              const week = Math.floor(days / 7) + 1;
              weeklyFeed[week] =
                (weeklyFeed[week] || 0) + (r.feed_consumed_kg || 0);
            }
            ...
            if (weights) {
              for (const w of weights) {
                ...
                const days = Math.floor(
                  (wDate.getTime() - firstDate.getTime()) / 86400000,
                );
                const week = Math.floor(days / 7) + 1;
            ...
            let cumFeed = 0;
            const weeks = Object.keys(weeklyFeed)
              .map(Number)
              .sort((a, b) => a - b);
            for (const week of weeks) {
              ...
              if (weeklyWeights[week]?.length) {
                avgW =
                  weeklyWeights[week].reduce((a, b) => a + b, 0) /
                  weeklyWeights[week].length;
              } else {
                for (let w = week; w >= 1; w--) {
                  if (weeklyWeights[w]?.length) {
                    avgW =
                      weeklyWeights[w].reduce((a, b) => a + b, 0) /
                      weeklyWeights[w].length;
                    break;
                  }
                }
              }
              ...
              const fcr =
                effectiveW > 0
                  ? Math.round((cumFeed / effectiveW) * 100) / 100
                  : 0;
              fcrRows.push({
                week,
                avgWeight: effectiveW,
                totalFeed: cumFeed,
                fcr,
              });
            }
        ...
        try {
          let wtQuery = supabase
            ...
            .limit(100);
          if (selectedFlock !== "all")
            wtQuery = wtQuery.eq("flock_id", selectedFlock);
          const { data: wt } = await wtQuery;
            ...
            setWeightData(
              wt
                .map((w) => ({
                  date:
                    w.sample_date ||
                    (w.created_at ? w.created_at.split("T")[0] : ""),
  ...
  useEffect(() => {
    if (contentRef.current && !loading) {
      const ctx = gsap.context(() => {
        gsap.fromTo(
          contentRef.current!.querySelectorAll(".insight-card"),
          { y: 30, opacity: 0 },
          {
            y: 0,
            opacity: 1,
  ...
  const avgFCR =
    fcrData.length > 0
      ? fcrData.reduce((s, f) => s + f.fcr, 0) / fcrData.length
      : 0;
  const bestFCR =
    fcrData.length > 0 ? Math.min(...fcrData.map((f) => f.fcr)) : 0;
  const latestFCR = fcrData.length > 0 ? fcrData[fcrData.length - 1].fcr : 0;
  const latestWeight =
    weightData.length > 0
      ? weightData[weightData.length - 1].weight
      : fcrData.length > 0
        ? fcrData[fcrData.length - 1].avgWeight
        : 0;
      ...
      <div className="mb-6 md:mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">
            Feed Conversion Ratio analysis and bird growth curves
          </p>
        </div>
        ...
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            {[
              {
                label: "Avg FCR",
                value: avgFCR > 0 ? avgFCR.toFixed(2) : "—",
                icon: "calculate",
                color: fcrColor(avgFCR),
              },
              {
                label: "Best FCR",
                value: bestFCR > 0 ? bestFCR.toFixed(2) : "—",
                icon: "emoji_events",
                color: fcrColor(bestFCR),
              },
              {
                label: "Latest FCR",
                value: latestFCR > 0 ? latestFCR.toFixed(2) : "—",
                icon: "trending_up",
                color: fcrColor(latestFCR),
              },
              {
                label: "Latest Weight",
                value: latestWeight > 0 ? `${latestWeight.toFixed(2)} kg` : "—",
                icon: "monitor_weight",
                color: "text-sky-600",
              },
            ].map((kpi) => (
              <div
                key={kpi.label}
                className="insight-card bg-white rounded-xl border border-slate-200/80 p-4 shadow-sm"
              >
                <div className="w-9 h-9 rounded-lg bg-slate-50 flex items-center justify-center mb-2">
                  <span
                    className={`material-symbols-outlined text-base ${kpi.color}`}
                  >
                    {kpi.icon}
                  </span>
                </div>
                <p
                  className={`font-mono text-xl font-bold ${kpi.color} tracking-tighter`}
                >
                  {kpi.value}
                </p>
            ...
            {fcrData.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-[10px] text-slate-300">
                  Submit keeper reports with feed and weight data, then approve
                  them.
                </p>
              </div>
            ) : (
          ...
          <div className="insight-card bg-white rounded-xl border border-slate-200/80 p-4 md:p-6 shadow-sm">
            ...
            {weightData.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-[10px] text-slate-300">
                  Weight data appears from keeper daily reports (weight samples
                  section).
                </p>
              </div>
            ) : (
              <>
                ...
                <p className="text-[10px] text-slate-400 mt-2">
                  {weightData.length} weight samples recorded
                </p>
              </>
            )}
          </div>
