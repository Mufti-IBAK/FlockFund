...
export default function AccountantSalaries() {
  ...
  async function handlePaySalary(member: StaffProfile) {
    ...
    if (!confirm(`Confirm salary payment of ₦${member.salary_amount.toLocaleString()} to ${member.full_name} for ${currentMonth} ${currentYear}?`)) return;
                  ...
                  <tr key={s.id} className="hover:bg-slate-50/50 transition-all">
                    ...
                    <td className="px-6 py-4">
                      {s.bank_name ? (
                        ...
                        <span className="text-[10px] text-rose-500 font-bold uppercase italic">Missing Details</span>
                      )}
                    </td>
