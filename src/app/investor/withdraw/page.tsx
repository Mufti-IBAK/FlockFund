...
export default function WithdrawPage() {
  ...
  const [settings, setSettings] = useState({ cost_per_bird: 4250, selling_price_per_bird: 7500 });
  
  // Modals / states
  const [selectedCapitalInvestment, setSelectedCapitalInvestment] = useState<string>('');
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const [invResult, wdResult, profileResult, settingsResult] = await Promise.all([
          supabase.from('investments').select('*, flocks(selling_price_per_bird, cost_per_bird)').eq('investor_id', user.id).in('status', ['active', 'completed']),
  ...
  const totalAccruedProfit = investments.reduce((acc, inv) => {
    ...
    const sell = inv.flocks?.selling_price_per_bird || settings.selling_price_per_bird;
    // Profit = (Revenue - Cost) * rounds * 70% investor share
    const profitPerBird = (sell - cost) * 0.70;
  ...
  const eligibleCapital = investments.filter((i) => i.round_count >= 3 && i.status === 'active');
  const lockedCapital = investments.filter((i) => i.round_count < 3 && i.status === 'active');

  async function handleProfitWithdraw() {
    if (profitWithdrawAmount <= 0 || profitWithdrawAmount > availableProfit || !bankInfo) return;
    setRequesting(true);
    try {
      const { createClient } = await import('@/lib/supabase/client');
      const supabase = createClient();
      ...
      const payoutRes = await fetch('/api/payments/payout', {
        ...
        body: JSON.stringify({ investor_id: user.id, amount: profitWithdrawAmount, withdrawal_id: wd.id }),
      });
      ...
      if (payoutData.success) {
        ...
        alert(`Profit withdrawal initiated! Reference: ${payoutData.reference}.`);
      } else {
        alert(`Request saved but automated transfer failed: ${payoutData.error || 'Unknown error'}`);
  ...
  async function handleCapitalWithdraw() {
    ...
    try {
      ...
      const payoutRes = await fetch('/api/payments/payout', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ investor_id: user.id, amount: inv.amount_invested, withdrawal_id: wd.id }),
      });
      const payoutData = await payoutRes.json();
      ...
      if (payoutData.success) {
        ...
        alert(`Capital liquidation initiated! Reference: ${payoutData.reference}.`);
      } else {
  ...
  return (
    <div ref={pageRef}>
      <div className="mb-6 fade-in">
        ...
        <p className="text-slate-400 text-sm mt-1">Extract your accrued profit or liquidate mature capital</p>
      </div>
      ...
      {!bankInfo ? (
        ...
        <div className="fade-in bg-emerald-50 border border-emerald-200 rounded-xl p-4 mb-6 flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-bold text-emerald-800">Bank Approved</p>
              <p className="text-xs text-emerald-600">{bankInfo.bank_name} • ****{bankInfo.account_number.slice(-4)}</p>
            </div>
          </div>
          <Link href="/investor/settings" className="px-3 py-1.5 bg-emerald-100 text-emerald-800 rounded-lg text-[10px] font-bold uppercase tracking-wider hover:bg-emerald-200 transition-colors">Update</Link>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6 mb-6">
        ...
        <div className="fade-in bg-white rounded-xl border border-slate-200/80 p-5 shadow-sm flex flex-col">
          ...
          <p className="text-xs text-slate-400 mb-6">Withdraw your accrued returns. Available at any time after a flock cycle completes.</p>
          ...
          <div className="bg-slate-50 border border-slate-100 rounded-xl p-4 mb-6 flex-1 flex flex-col justify-center items-center">
            ...
            <p className="font-mono text-3xl font-extrabold text-primary tracking-tighter">₦{availableProfit.toLocaleString()}</p>
          </div>
          ...
          <div className="space-y-3">
            <div>
              ...
              <input 
                ...
                onChange={(e) => setProfitWithdrawAmount(Math.min(availableProfit, Number(e.target.value)))}
                placeholder="0.00"
            ...
            <button 
              ...
              disabled={requesting || profitWithdrawAmount <= 0 || profitWithdrawAmount > availableProfit || !bankInfo}
              ...
              {requesting ? <div className="w-4 h-4 border-2 border-primary/30 border-t-primary rounded-full animate-spin" /> : <span className="material-symbols-outlined text-lg">account_balance_wallet</span>}
              Withdraw Profit
            </button>
          </div>
        </div>
        ...
        <div className="fade-in bg-white rounded-xl border border-slate-200/80 p-5 shadow-sm flex flex-col">
          <h3 className="text-sm font-heading font-bold text-slate-800 uppercase tracking-wider mb-2 flex items-center gap-2">
            <span className="material-symbols-outlined text-indigo-500 text-base">exit_to_app</span>
            Capital Liquidation
          </h3>
          <p className="text-xs text-slate-400 mb-6">Fully extract your principal capital. Requires a minimum maturation of 3 completed operational cycles.</p>
          
          <div className="flex-1">
            <div className="mb-4">
              <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-2">Eligible Capital Portfolios</label>
              {eligibleCapital.length === 0 ? (
                <div className="bg-slate-50 border border-dashed border-slate-200 rounded-lg p-6 text-center">
                   <span className="material-symbols-outlined text-slate-300 text-3xl mb-2">lock_clock</span>
                   <p className="text-sm text-slate-400 font-medium">No investments have met the 3 cycle requirement yet.</p>
                </div>
              ) : (
                <div className="space-y-2 max-h-[140px] overflow-y-auto pr-1">
                  {eligibleCapital.map((inv) => (
                    <button key={inv.id} onClick={() => setSelectedCapitalInvestment(inv.id)}
                      className={`w-full text-left p-3 rounded-lg border-2 transition-all ${
                        selectedCapitalInvestment === inv.id ? 'border-primary bg-primary/5' : 'border-slate-200 hover:border-slate-300 bg-white'
                      }`}>
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-sm font-bold text-primary">{inv.birds_owned} birds</p>
                          <p className="text-[10px] text-slate-400">Cycles: {inv.round_count} • {inv.id.slice(0, 8)}</p>
                        </div>
                        <span className="font-mono font-bold text-indigo-600 text-sm">₦{inv.amount_invested.toLocaleString()}</span>
                      </div>
                    </button>
                  ))}
                </div>
            ...
            {lockedCapital.length > 0 && (
               <div className="mb-4">
                 <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-2">Immature Portfolios ({lockedCapital.length})</p>
                 <div className="space-y-2">
                   {lockedCapital.map((inv) => (
                     <div key={inv.id} className="p-2.5 rounded-lg border border-slate-200 bg-slate-50 flex items-center justify-between opacity-70">
                        <div>
                          <p className="text-xs font-bold text-slate-500">{inv.birds_owned} birds</p>
                          <p className="text-[9px] text-slate-400">{inv.round_count}/3 cycles required</p>
                        </div>
          ...
          <button 
            ...
            {requesting ? <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <span className="material-symbols-outlined text-lg">savings</span>}
            Liquidate Capital
          </button>
        </div>
      ...
      {withdrawals.length > 0 && (
        <div className="mt-2 fade-in bg-white rounded-xl border border-slate-200/80 p-4 md:p-5 shadow-sm">
          <h3 className="text-sm font-heading font-bold text-primary uppercase tracking-wider mb-4">Transaction Ledger</h3>
          <div className="space-y-2">
            {withdrawals.map((wd) => (
              <div key={wd.id} className="flex items-center justify-between py-3 px-4 rounded-lg border border-slate-100 bg-slate-50/50">
                <div className="flex items-center gap-4">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${wd.payment_reference ? 'bg-indigo-100 text-indigo-600' : 'bg-accent/20 text-accent'}`}>
                     <span className="material-symbols-outlined text-lg">{wd.payment_reference ? 'exit_to_app' : 'payments'}</span>
                  </div>
                  <div>
                    <p className="text-sm font-bold text-primary">₦{wd.amount.toLocaleString()}</p>
                    <p className="text-[10px] text-slate-400">{new Date(wd.created_at).toLocaleDateString()} • {wd.payment_reference ? `Capital Liq.` : `Profit Extr.`}</p>
                  </div>
                </div>
