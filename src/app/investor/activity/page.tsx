    ...
    async function load() {
      try {
        ...
        const [reportsRes, incidentsRes] = await Promise.all([
          supabase.from('farm_reports').select('*').eq('status', 'approved').order('created_at', { ascending: false }).limit(20),
          supabase.from('incident_reports').select('*').order('updated_at', { ascending: false }).limit(20)
        ]);

        const combined: any[] = [
          ...(reportsRes.data || []).map(r => ({ ...r, type: 'report', sortDate: new Date(r.created_at).getTime() })),
          ...(incidentsRes.data || []).map(i => ({ ...i, type: 'incident', sortDate: new Date(i.updated_at).getTime() }))
        ];
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {reports.map((r: any) => {
            ...
            let color = r.mortality_count > 0 ? 'bg-rose-100 text-rose-500' : 'bg-emerald-100 text-emerald-600';
            let detail = '';
            ...
            if (isIncident) {
              if (r.status === 'received') {
                ...
                title = isRisk ? `Risk Found: ${r.title}` : `Incident Resolved: ${r.title}`;
              ...
              <div key={r.id} className="feed-item bg-white rounded-2xl border border-slate-200/80 p-5 shadow-sm hover:shadow-md transition-all duration-300">
                <div className="flex items-center gap-4">
                  <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${color.split(' ')[0]}`}>
                    <span className={`material-symbols-outlined text-lg ${color.split(' ')[1]}`}>
                      {icon}
                    </span>
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap gap-x-4 gap-y-1 mt-1 text-[10px] text-slate-400">
                      ...
                      {isIncident && r.description && <span>{r.description.slice(0, 80)}...</span>}
                      {detail && <span className="text-slate-600 font-medium italic">Outcome: {detail}</span>}
                    </div>
                  </div>
                  <span className="text-[10px] text-slate-300 font-mono whitespace-nowrap">{new Date(r.report_date || r.updated_at || r.created_at).toLocaleDateString()}</span>
                </div>
              </div>
            );
          })}
